...
function weatherDetails(info) {
  ...
  document.getElementById("derece").innerText =
    Math.round(info.main.temp) + "°C";
  document.querySelector(
    ".durum-icon"
  ).innerHTML = `<img src="https://openweathermap.org/img/wn/${info.weather[0].icon}.png" alt="Durum İkonu">`;
  document.getElementById("durum-açıklama").innerText =
    info.weather[0].description;
  document.getElementById("nem-label").innerText =
    "Nem: " + info.main.humidity + "%";
  document.getElementById("rüzgar-hızı-label").innerText =
    "Rüzgar Hızı: " + info.wind.speed + " km/s";
...
function dataWrite() {
  ...
  fetch(api)
    ...
    .catch((error) => {
      ...
      showPopup(
        "Hata",
        "Veri çekilirken bir sorun oluştu. API anahtarınızı veya internetinizi kontrol edin"
      );
    });
}
